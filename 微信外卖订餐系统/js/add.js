$(function () {
    ...
    $(".foot").click(function () {
		...
		if(sessionStorage.getItem("isLogin") == 0){
    		newCartId = sessionStorage.getItem("newCartId");
    	}else{
    		newCartId = localStorage.getItem("newCartId");
    	}
		...
		if($(".list-content ul li").length <= 0){
			...
			addtr += '<div class="btn">';
			addtr += '<button class="ms2" id="ms2'+newCartId+'" onclick="sub('+newCartId+')" style="display: inline-block;"></button>';
			addtr += '<i class="li_acount">'+acount+'</i>';
			addtr += '<button class="ad2" id="ad2'+newCartId+'" onclick="add('+newCartId+')"></button>';
		...
		if (flag == false) {
			...
			addtr += '<div><span>￥</span><span class="accountPrice">'+sum+'</span></div>'	;					
			addtr += '<div class="btn">';
			addtr += '<button class="ms2" id="ms2'+newCartId+'" onclick="sub('+newCartId+')" style="display: inline-block;"></button><i class="li_acount">'+acount+'</i><button class="ad2" id="ad2'+newCartId+'" onclick="add('+newCartId+')"></button><i class="price" style="display: none;">'+price+'</i>';
			addtr += '</div>';						
			addtr += '</li>';						
			$(".list-content ul").append(addtr);
		}
    ...
    $(".empty").click(function(){
    	if(sessionStorage.getItem("isLogin") == 0){
    		...
    		$.ajax({
				type:"get",
				url:"http://localhost:9527/joy-cart/cart/clearCart",
